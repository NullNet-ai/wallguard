...
pub fn DeviceTunnels() -> impl IntoView {
    ...
    let session_id = move || {
        query.read().get("session").filter(|s| !s.is_empty())
    };

    let tunnel_type = move || query.read().get("type").unwrap_or_default();

    let rdp_width  = move || query.read().get("w").and_then(|s| s.parse::<u32>().ok()).unwrap_or(1280);
    let rdp_height = move || query.read().get("h").and_then(|s| s.parse::<u32>().ok()).unwrap_or(720);
            ...
            spawn_local(async move {
                match crate::api::tunnels::open_rdp(id, W, H, 30, 4000).await {
                    Ok(resp) => {
                        ...
                        nav(
                            &format!(
                                "/devices/{id}/tunnels?session={}&type=rdp&w={W}&h={H}&ws={}",
                                resp.session_id,
                                js_sys::encode_uri_component(&ws_url),
                            ),
                            Default::default(),
                    ...
                    fallback=move || view! {
                        <div class="tunnel-open-panel">
                            ...
                            <div class="tunnel-actions">
                                <button class="btn btn-primary"   on:click=open_ssh.clone()>"Open SSH Tunnel"</button>
                                <button class="btn btn-secondary" on:click=open_tty.clone()>"Open TTY Tunnel"</button>
                                <button class="btn btn-secondary" on:click=open_rdp.clone()>"Open Remote Desktop"</button>
                            </div>
                        </div>
                    }
                >
                    <div class="tunnel-session">
                        <div class="session-info">
                            ...
                            <code class="session-id">{move || session_id().unwrap_or_default()}</code>
                            <span class="session-type-label">"Type:"</span>
                            <span class="session-type">{move || tunnel_type().to_uppercase()}</span>
                        </div>
                            ...
                            if tt == "rdp" {
                                ...
                                view! {
                                    <div class="rdp-wrapper">
                                        <crate::components::RemoteDesktop ws_url=url width=w height=h/>
                                    </div>
                                }.into_any()
                            } else {
                                view! {
