...
use crate::http_api::tty_gateway_v2::session::{
...
async fn internal_relay_impl(
    ...
    tokio::select! {
        _ = from_users_to_channel(data_receiver, channel_writer) => {
            log::debug!("TTY Internal Relay: Channel to TTY relay finished");
        }
        _ = from_channel_to_users(channel_reader, data_sender) => {
            log::debug!("TTY Internal Relay: TTY to Channel relay finished");
        }
        _ = terminate.recv() => {
            log::debug!("TTY Internal Relay: TERM singal received");
        }
    }
    ...
    let Ok(token) = context.sysdev_token_provider.get().await else {
        log::error!("TTY Internal Relay: failed to acquire token");
        return;
    };

    if context
        .datastore
        .update_tty_session_status(&token.jwt, &session_id, TtySessionStatus::Terminated, false)
        .await
        .is_err()
    {
